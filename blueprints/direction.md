...
토큰을 소모하는 설명문은 완전히 생략하고, 즉시 /combo-run 명세에 따른 pre-flight-check 파일 목록 선언부터 시작하라.

만약 이 명령 이후에도 코드 원본이 아닌 '설명, 다짐, 가치 검토'의 문장이 단 한 줄이라도 섞여 나올 경우, 프로덕션 빌드 실패로 간주한다. 즉시 코딩을 수행하라.