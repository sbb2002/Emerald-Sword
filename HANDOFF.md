...
## 🚨 긴급 — 계정이 어정쩡한 상태 (2026-06-26)

`/emergency-stop` 청산이 **토큰 분당 제한 버그로 실패**했고, 그 버그는 `f23a7ce` 로 수정·푸시됨(재배포됨). 하지만 그때 `is_paused=True` 만 걸리고 **청산은 안 됐다**:
- **`is_paused = True`** (청산 전 early-pause 가 먼저 걸림 — 정상 안전장치)
- **QQQM 324주 그대로 보유** (청산 미완)

### 사용자가 할 일 (재배포 완료 후)
1. `/status` — 서버 ✅ + QQQM 324주 보유 확인.
2. **`/emergency-stop` 재실행** — 수정본은 토큰 1회만 쓰므로 정상 청산됨. (직전 KIS 호출과 **1~2분 텀** 둘 것 — 토큰 분당 제한.)
3. 청산 후에도 `is_paused=True` 유지(정상). 자동매매 재개하려면 `/resume`.
> 청산을 원치 않고 그냥 보유 유지할 거면 `/resume` 만 하면 정상 운용 복귀.
