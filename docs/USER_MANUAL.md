# 사용자 매뉴얼 — 복합모멘텀 QQQM/GLDM 봇 (현재 구현 기준)

> **이 문서의 목적**: Phase A·B·C 구현이 끝난 **지금 코드 상태**로 배포하고, **모의투자(virtual)** 로 안전하게 검증하기까지의 실전 절차서.
> 계정·키 발급 같은 *사람의 사전 준비*는 [`blueprints/SETUP_사용자_준비절차.md`](../blueprints/SETUP_사용자_준비절차.md) 가 담당한다(그 문서는 그대로 유효). 이 매뉴얼은 그 다음 단계 — **배포 → 어댑터 검증 → 모의 테스트 → 운영** 을 다룬다.
...
| `/signal` 응답이 안 옴(에러) | KIS 호출 실패 또는 월말 종가 13개 미만 | §4 검증, `get_daily_closes` 충분한 기간 반환 확인 |
